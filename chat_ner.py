...
if __name__ == '__main__':
    ...
    base_model = model_class.from_pretrained(
        ...
        device_map='auto',
        trust_remote_code=True if args.model_name == "bloom" else False
    )
    end = time.time()
    print("加载模型耗时：{}分钟".format((end-start)/60))

    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_path, trust_remote_code=True if args.model_name == "bloom" else False)
